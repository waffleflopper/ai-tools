---
name: prune-tests
description: Audit and prune tests that protect incidental code or presentation instead of behavior. Use when the user asks to find, delete, or rewrite tautological tests, change-detector tests, geometry assertions, incidental-copy assertions, or similarly brittle coverage.
---

# Prune tests

Delete by default. A suspect test earns survival only when it proves consequential behavior through a stable seam. Rewriting is an exception, not a compromise.

## Set the boundary

Pin the audit scope from the user's request: a diff, pull request, path, suite, issue, or the whole repository. Keep that boundary fixed. If the request supplies no usable boundary, ask one focused question before deleting anything.
...
Classify individual tests, not whole files. Remove a file only after every test in it has a disposition.
...
An expected value must be able to disagree with the implementation. Use an independently known result when a real behavior deserves coverage. Otherwise delete the test.
...
Delete these tests. If a real contract is hidden inside one, extract the smallest behavioral test that can fail for the contract itself.
...
Delete assertions that merely confirm a rendered prop, or that an element remains focused after no consequential event.
...
Keeping or rewriting requires a written justification with evidence for every numbered item in the behavioral bar. Missing any item means the justification is weak. Delete the test instead.
...
Delete or rewrite the classified tests. Clean up imports, fixtures, helpers, snapshots, screenshots, and setup code left unused by those decisions. Do not change production behavior merely to preserve a test, and do not weaken unrelated behavioral coverage or test configuration.
...
List every `DELETE`, `REWRITE`, and `KEEP` with its path and reason. Include the full strong justification for every `KEEP` and `REWRITE`, the commands run, failures, and any policy conflict. State explicitly when no acceptable rewrite existed.
