...
A subtle scale-down can give buttons tactile feedback when it fits the design. `scale(0.96)` is an optional starting point, not a required replacement for an existing token. Use an interruptible transition so a release mid-press returns smoothly.

Not every button needs it. Reuse any existing motion opt-out; add an API such as the example `static` prop only when the current component needs both behaviors.
