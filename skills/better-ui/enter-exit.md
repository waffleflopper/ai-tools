...
Optional recipes for staged entrances and exits, when they serve the requested design. Preserve existing motion tokens; the timings, offsets, and blur values below are examples. Honor reduced motion and keep content available without the animation. For interactive feedback see [animations.md](animations.md); for icon swaps see [icon-transitions.md](icon-transitions.md).
