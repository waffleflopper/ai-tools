...
For a contextual icon change that benefits from motion, a cross-fade is one option. Reuse the established transition; add scale or blur only when it helps. An immediate swap is appropriate for frequent interactions or reduced motion.
...
The examples use scale `0.25` → `1`, opacity `0` → `1`, blur `4px` → `0px`, and a `0.3s` spring with zero bounce. These are optional recipe values where no suitable motion tokens exist. Preserve approved values, omit unnecessary effects, and provide reduced-motion behavior and a lasting static state cue.
