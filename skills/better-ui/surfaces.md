...
For closely nested surfaces with an even inset, concentric radii are a useful starting point:
...
Where an existing depth treatment causes a visible problem, consider a subtle `box-shadow` using the project's elevation tokens. Shadows use transparency, so they adapt to any background where solid borders do not. That matters most over images or multiple background colors, which a fixed border color was never designed for.
...
When an image needs edge separation, try an existing border or outline token. A low-opacity `1px` outline is an optional starting point; do not add one to every image by default.

### Neutral outline recipe

Where the project has no suitable token, try pure black in light mode (`oklch(0 0 0 / 0.1)`) and pure white in dark mode (`oklch(1 0 0 / 0.1)`). Inspect the edge against the actual image and surrounding surface. Preserve a project palette value when it already gives the intended separation.
...
The example uses `outline-black/10` and `outline-white/10`; substitute approved tokens when available.
