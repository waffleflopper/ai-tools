...
Write fixes in the project's styling system and reuse its approved type scale and tokens. Aesthetic values below and in references are starting points where the project lacks a suitable convention; they do not override a working design system. Report readability or behavior problems with evidence. The [cheat sheet](css-cheat-sheet.md) maps declarations to Tailwind equivalents.
...
iOS Safari can zoom when an input's text is smaller than `16px`. Reuse the existing mobile input pattern. If none resolves the issue, prefer readable `16px` mobile text; reserve transform compensation for an explicit design requirement. Resolve this routine choice from the design and task without asking by default:
...
Preserve the project's font-smoothing choice. If rendered text needs adjustment, compare legibility before changing it: antialiasing can make light text thinner. Apply a chosen smoothing convention at the root rather than per component. Tailwind's `antialiased` sets `-webkit-font-smoothing: antialiased` and `-moz-osx-font-smoothing: grayscale`.
...
**Verification.** Without a browser, inspect declared size, weight, line-height, measure, font loading, and truncation rules; identify likely issues without claiming computed or rendered results. With a browser, inspect computed styles and the rendered hierarchy, then resize with realistic content to check wrapping, widows, truncation, and mobile input behavior. Mark unavailable rendered checks `Not verified`.
