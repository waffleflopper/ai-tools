...
A skill the active platform permits the agent to select automatically. Its description helps the agent decide when it applies. It may also be requested explicitly. Metadata visibility and cross-skill loading depend on the platform.

### User-Invoked

A skill selected explicitly by the user rather than automatically. Preserve its description and use the active platform's invocation controls. Explicit invocation does not itself grant permission for unrelated actions.

### Description

The machine-readable summary of what a skill does and when it applies. Keep it concise and accurate. Invocation policy is a separate platform setting, not a consequence of deleting this field.
...
The instruction text and metadata the platform includes in a model request. The amount depends on discovery, invocation, and reference loading; do not assume a universal per-turn cost or zero cost for explicit-only skills.
...
A skill that helps select related workflows. It may load a target only when the active platform and user scope permit that action; otherwise it identifies the next skill for the user.
...
Reference outside the skill entrypoint that another skill can read when relevant and accessible. Use it to share maintained guidance without duplication. Reading reference material and invoking a workflow are different actions.
...
A prohibition can leave the intended action unclear. Pair it with a useful alternative when necessary. Keep explicit prohibitions for real scope, safety, and correctness boundaries; assess their effectiveness through observed behavior rather than a universal claim that negation backfires.
...
Assess a proposed wording change on representative tasks. If a reminder has no useful effect, remove it or replace it with a concrete outcome or stopping condition. Stronger adjectives are not evidence of better adherence.
